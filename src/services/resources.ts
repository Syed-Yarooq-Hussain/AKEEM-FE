...
export type FileRecord={id:number;processingStatus?:'pending'|'processing'|'ready'|'failed';processingError?:string|null;projectId?:number;name?:string;originalName?:string;filename?:string;mimeType?:string;size?:number;sizeBytes?:number;url?:string;createdAt?:string}
...
export const getFile=(id:number)=>apiRequest<FileRecord>(`/files/${id}`)
export const reprocessFile=(id:number)=>apiRequest<FileRecord>(`/files/${id}/process`,{method:'POST'})
