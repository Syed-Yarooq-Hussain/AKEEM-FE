...
export async function crmList(resource:'contacts'|'companies'|'deals'){
 const result:ApiRecord[]=[];let page=1;
 while(true){const data=await apiRequest<ListData>(`/crm/${resource}?page=${page}&limit=100`);result.push(...items(data));if(!data.pagination||page>=data.pagination.totalPages)return result;page++}
}
...
export const updateDealStage=(id:number,stageId:number)=>apiRequest(`/crm/deals/${id}/stage`,{method:'PATCH',body:JSON.stringify({stageId})})
export const financeOverview=(projectId?:number)=>apiRequest<ApiRecord>(`/finance/overview${projectId?`?projectId=${projectId}`:''}`)
export async function financeList(resource:'transactions'|'invoices'|'budgets'|'cash-flow'|'reports',projectId?:number){const data=await apiRequest(`/finance/${resource}${projectId?`?projectId=${projectId}`:''}`);return items(data).length?items(data):data}
